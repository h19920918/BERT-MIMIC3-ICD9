...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.vocab_size != config.redefined_vocab_size:
            self.redefined_word_embeddings = nn.Embedding(config.redefined_vocab_size, config.hidden_size, padding_idx=0)
            self.word_modification = True
        else:
            self.word_embeddings = nn.Embedding(config.vocab_size, config.hidden_size, padding_idx=0)
            self.word_modification = False

        if config.max_position_embeddings != config.redefined_max_position_embeddings:
            self.redefined_position_embeddings = nn.Embedding(config.redefined_max_position_embeddings, config.hidden_size)
            self.position_modification = True
        else:
            self.position_embeddings = nn.Embedding(config.max_position_embeddings, config.hidden_size)
            self.position_modification = False
    ...
    def forward(self, input_ids, token_type_ids=None, position_ids=None):
        ...
        if self.word_modification:
            words_embeddings = self.redefined_word_embeddings(input_ids)
        else:
            words_embeddings = self.word_embeddings(input_ids)

        if self.position_modification:
            position_embeddings = self.redefined_position_embeddings(position_ids)
        else:
...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if not config.model in ['bert-small-caml', 'bert-tiny-caml', 'bert-tiny-parallel3-caml', 'bert-tiny-parallel4-caml']:
...
class BertForQuestionAnswering(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None,
        ...
        pooler_loss = pooler_output.view(-1).sum(-1) * 0
        ...
        if start_positions is not None and end_positions is not None:
            ...
            total_loss = (start_loss + end_loss + pooler_loss) / 2
            outputs = (total_loss,) + outputs
...
class BertForMedical(BertPreTrainedModel):
    def __init__(self, config):
        super(BertForMedical, self).__init__(config)
        self.lmbda = 0
        self.Y = config.Y
        self.gpu = config.gpu
        self.last_module = config.last_module
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None,
        ...
        if self.last_module == 'basic':
            ...
            x = outputs[0]
            x = self.dropout(x)
            x = x * attention_mask.float().unsqueeze(2)

            # Soft-attention
            key = self.key(x).squeeze(2)
            key = key * attention_mask.float() + (attention_mask == 0).float() * -1e+15
            key = F.softmax(key, dim=1).unsqueeze(1)
            value = self.value(x)
        ...
        if labels is not None:
            if desc_data is not None:
                ...
                diffs = None
            loss = self._get_loss(logits, labels, diffs, pos_labels)
...
class BertWithCAMLForMedical(BertPreTrainedModel):
    def __init__(self, config):
        super(BertWithCAMLForMedical, self).__init__(config)
        self.config = config
        self.lmbda = 0
        self.Y = config.Y
        self.gpu = config.gpu
        ...
        if self.embed_file:
            ...
            W = torch.Tensor(extract_wvs.load_embeddings(self.embed_file))
...
class BertTinyParallel3WithCAMLForMedical(BertPreTrainedModel):
    def __init__(self, config):
        super(BertTinyParallel3WithCAMLForMedical, self).__init__(config)
        self.config = config
        self.lmbda = 0
        self.Y = config.Y
        self.gpu = config.gpu
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None,
        ...
        outputs2 = self.bert2(input_ids, \
                            attention_mask, \
                            inputs_embeds=x, \
                           )

        outputs3 = self.bert3(input_ids, \
                            attention_mask, \
                            inputs_embeds=x, \
                           )
        ...
        if self.last_module == 'basic':
            ...
            alpha = F.softmax(alpha, dim=2)
            m = alpha.matmul(x)
            logits1 = self.bert_classifier.weight.mul(m).sum(dim=2).add(self.bert_classifier.bias)
...
class BertTinyParallel4WithCAMLForMedical(BertPreTrainedModel):
    def __init__(self, config):
        super(BertTinyParallel4WithCAMLForMedical, self).__init__(config)
        self.config = config
        self.lmbda = 0
        self.Y = config.Y
        self.gpu = config.gpu
